...
class TaskGenerator():
    ...
    def __init__(self, ns, state_collector, robot_radius):
        ...
        self.__move_base_status_id = ""                 # recent id of move_base_rgring status
        ...
        # Subscriber
        self.__goal_status_sub = rospy.Subscriber("%s/move_base_rgring/status" % self.NS, GoalStatusArray,
                                                  self.__goal_status_callback, queue_size=1)
        self.__map_sub = rospy.Subscriber("%s/map" % self.NS, OccupancyGrid, self.__map_callback)
        self.__path_sub = rospy.Subscriber("%s/move_base_rgring/NavfnROS/plan" % self.NS, Path, self.__path_callback)
        # self.__path_sub = rospy.Subscriber("%s/move_base_rgring/GlobalPlanner/plan" % self.NS, Path, self.__path_callback)
